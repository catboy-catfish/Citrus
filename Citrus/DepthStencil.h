...
public:
	DepthStencil(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, int width, int height, UINT msaaQuality, bool msaaEnabled, Usage usage);
	void BindAsDepthStencil(ID3D11DeviceContext* pContext) const noexcept;
	void Clear(ID3D11DeviceContext* pContext) const noexcept;
	void BindSwapBuffer(ID3D11DeviceContext* pContext, ID3D11RenderTargetView* pRtv) noexcept;
