...
bool ChoiWin::InitializeWindow(std::string wndName, std::string className, HINSTANCE hInstance, const int width, const int height) noexcept
{
	try
	{
		...
		wc.hIconSm = static_cast<HICON>(LoadImage(hInstance, MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, 128, 128, 0)); //icon in the taskbar 
		...
		SetWindowLong(hwnd, GWL_STYLE, GetWindowLong(hwnd, GWL_STYLE) & ~WS_MINIMIZEBOX & ~WS_SIZEBOX & ~WS_MAXIMIZEBOX);
